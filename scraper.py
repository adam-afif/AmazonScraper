...
API_KEY = "9ef27d00-5303-476a-9f61-5209b9b7ee24"
                ...
                for div in divs:
                    ...
                    if h2 and h2.text.strip() and h2.text.strip() and parsable:
                        ...
                        if symbol_presence is not None:
                            ...
                            price_match = re.search(r"[\d.,]+", price_present)
                            price = float(price_match.group().replace(",", "")) if price_match else 0.0


                            real_price_text = prices[1].text if len(prices) > 1 else str(price)
                            real_price_match = re.search(r"[\d.,]+", real_price_text)
                            real_price = float(real_price_match.group().replace(",", "")) if real_price_match else price
...
def parse_product(product_object, location="us", retries=3):
    ...
    while tries <= retries and not success:
        try:
            scrapeops_url = get_scrapeops_url(product_url, location=location)
            resp = requests.get(scrapeops_url, headers=headers)
            if resp.status_code == 200:
                soup = BeautifulSoup(resp.text, "html.parser")
